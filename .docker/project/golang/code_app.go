...
const (
	...
	destinationFolder = "code_app"

	// Define the content folder
	baseProjectDir = "/var/www/code/app/Bundle"

	// Define the generated file prefix
	generatedFilePrefix = "PHP_"

	// Define file extensions to include (use "*" for any extension, ".php" for PHP files, ".go" for Go files)
	fileExtensions = ".php" // Change to "*" for any extension or ".php" for PHP files

	// Set this to true to ignore .json and .blade.php files
	ignoreSpecificFiles = true
)
...
func main() {
	...
	err = filepath.Walk(baseProjectDir, func(path string, info os.FileInfo, err error) error {
		...
		// Skip specific file types if the ignore flag is true
		if ignoreSpecificFiles && (strings.HasSuffix(info.Name(), ".json") || strings.HasSuffix(info.Name(), ".blade.php")) {
